...
import logo from '../public/assets/img/op-logo.svg';
import opCharacters from '../public/assets/img/op-characters.svg';
import opCharactersBig from '../public/assets/img/op-characters-big.png';
    ...
    return (
      <>
        <div className={`op-characters ${this.state.readAll ? "read-all" : ""}`}>
          <img className="op-characters__img" src={opCharacters.src} srcSet={`${opCharacters.src} 768w, ${opCharactersBig.src} 8530w`} alt="one piece main characters" />
          <div className="op-characters__gradient"></div>
        </div>
          <article className={`synopsis ${this.state.readAll ? "read-all" : ""}`}>
            <img src={logo.src} alt="one piece logo" className={`synopsis__logo ${this.state.readAll ? "visible" : ""}`} />
